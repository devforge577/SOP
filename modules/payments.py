...
def validate_payment_method(method: str) -> Tuple[bool, str]:
    ...
    if method.lower() not in valid_methods:
        return (
            False,
            f"Unsupported payment method: {method}. Supported: {', '.join(valid_methods)}",
        )
    return True, ""
...
def process_cash_payment(
    amount_paid: float, total_amount: float, sale_id: int
    ...
    try:
        ...
        logger.info(
            f"[CASH] Payment processed: Sale {sale_id}, Amount: GHS {amount_paid:.2f}, Change: GHS {change:.2f}"
        )
    ...
    try:
        ...
        if net_amount < float(total_amount):
            return (
                False,
                "",
                f"Amount after fee (GHS {net_amount:.2f}) is insufficient. Need GHS {total_amount:.2f}",
            )
        ...
        logger.info(
            f"[MOMO] Payment processed: Sale {sale_id}, Amount: GHS {amount_paid:.2f}, "
        ...
        return (
            True,
            transaction_id,
            f"MoMo payment confirmed. Transaction ID: {transaction_id}",
        )
...
def process_card_payment(
    ...
    try:
        ...
        logger.info(
            f"[CARD] Payment processed: Sale {sale_id}, Amount: GHS {amount_paid:.2f}, "
        ...
        return (
            True,
            transaction_id,
            f"Card payment approved. Reference: {transaction_id}",
        )
...
def process_bank_transfer(
    ...
    try:
        ...
        logger.info(
            f"[BANK] Payment initiated: Sale {sale_id}, Amount: GHS {amount_paid:.2f}, "
        ...
        return (
            True,
            transaction_id,
            f"Bank transfer initiated. Reference: {reference}. Awaiting verification.",
        )
...
def reconcile_payments(date: str = None) -> Tuple[bool, Dict[str, Any]]:
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            logger.info(
                f"[RECONCILIATION] {date}: Completed: GHS {report['totals']['completed']:.2f}, "
...
def process_refund(
    payment_id: int, reason: str, refund_amount: float = None
) -> Tuple[bool, str]:
    """
    ...
    try:
        with get_db_connection() as conn:
            ...
            if refund_amount is None:
                ...
                return (
                    False,
                    f"Refund amount (GHS {refund_amount:.2f}) exceeds original amount (GHS {original_amount:.2f})",
                )
            ...
            logger.info(
                f"[REFUND] Refunded GHS {refund_amount:.2f} for payment {payment_id}. Reason: {reason}"
            )

            return (
                True,
                f"Refund of GHS {refund_amount:.2f} processed. Reference: REFUND-{payment_id}",
            )
...
def process_payment(
    amount_paid: float, total_amount: float, sale_id: int, payment_method: str, **kwargs
    ...
    try:
        ...
        if payment_method.lower() == PaymentMethod.CASH.value:
            success, change, msg = process_cash_payment(
                amount_paid, total_amount, sale_id
            )
            transaction_id = (
                f"CASH-{sale_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
                if success
                else None
            )
            ...
            return process_momo_payment(
                amount_paid, total_amount, sale_id, phone, reference, provider
            )
            ...
            return process_card_payment(
                amount_paid, total_amount, sale_id, card_num, expiry, cvv, cardholder
            )
            ...
            return process_bank_transfer(
                amount_paid, total_amount, sale_id, account, reference
            )
