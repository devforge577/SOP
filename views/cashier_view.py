...
class CashierView(tk.Toplevel):
    ...
    def _build_payment_panel(self, parent):
        ...
        tk.Entry(
            self.momo_frame,
            textvariable=self.momo_ref_var,
            font=("Segoe UI", 11),
            relief="flat",
            bd=3,
            bg="#f7f8fa",
    ...
    def _on_payment_method_change(self, *args):
        ...
        if self.payment_var.get() == "momo":
            self.momo_frame.pack(
                fill="x", padx=12, pady=(0, 10), after=self.discount_entry_var
            )
        else:
    ...
    def _charge(self):
        ...
        if payment_method == "momo":
            ...
            if not phone:
                self._pay_msg(
                    "Enter customer phone number for MoMo payment.", error=True
                )
                return
            if not ref:
                self._pay_msg(
                    "Enter transaction reference for MoMo payment.", error=True
                )
                return
    ...
    def _set_pay_label_color(self, widget, color):
        if isinstance(widget, tk.Label) and str(widget.cget("textvariable")) == str(
            self.pay_msg_var
        ):
