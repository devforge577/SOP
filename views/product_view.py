...
class ProductView(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Button(
            ...
            relief="flat",
            cursor="hand2",
            command=self._show_low_stock_report,
        ...
        tk.Button(
            ...
            relief="flat",
            cursor="hand2",
            command=self._supplier_restock_dialog,
        ).pack(side="left", padx=2)
        ...
        tk.Button(
            ...
            relief="flat",
            cursor="hand2",
            command=self._bulk_import_dialog,
        ).pack(side="left", padx=2)
    ...
    def _build_right_panel(self, parent):
        right = tk.Frame(parent, bg="white", padx=20, pady=20, relief="flat", bd=1)
    ...
    def _supplier_restock_dialog(self):  # noqa: C901
        ...
        def populate_rows(show_all=False):
            ...
            for i, p in enumerate(products):
                ...
                stock_color = (
                    "#e74c3c"
                    if p.get("quantity", p.get("stock", 0)) == 0
                    else "#e67e22"
                    if p.get("quantity", p.get("stock", 0))
                    <= p.get("low_stock_alert", 5)
                    else "#27ae60"
                )
        ...
        def apply_restock():
            ...
            for product, qty_var, row_sup_var in restock_rows:
                ...
                ok, result = adjust_stock(
                    pid, qty, reason, self.current_user.get("user_id")
                )
                if ok:
                    applied += 1
                    logger.info(
