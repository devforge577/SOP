...
from modules.products import (
    ...
    get_categories, get_low_stock_products, restore_product,
    get_product_with_details, bulk_import_products
)
import logging
...
class ProductView(tk.Toplevel):
    ...
    Opened from the main dashboard by admin or manager.
    """
    ...
    def _build_ui(self):
        ...
        tk.Button(quick_actions, text="📊 Low Stock Report",
                  font=("Segoe UI", 9), bg="#0f3460", fg="white",
                  relief="flat", cursor="hand2",
                  command=self._show_low_stock_report).pack(side="left", padx=2)

        tk.Button(quick_actions, text="🚚 Supplier Restock",
                  font=("Segoe UI", 9), bg="#27ae60", fg="white",
                  relief="flat", cursor="hand2",
                  command=self._supplier_restock_dialog).pack(side="left", padx=2)

        tk.Button(quick_actions, text="📥 Bulk Import",
                  font=("Segoe UI", 9), bg="#0f3460", fg="white",
                  relief="flat", cursor="hand2",
                  command=self._bulk_import_dialog).pack(side="left", padx=2)

        self.show_inactive_var = tk.BooleanVar(value=False)
        tk.Checkbutton(topbar, text="Show Inactive Products",
                       variable=self.show_inactive_var,
                       command=self._load_products,
                       bg="#1a1a2e", fg="#8892b0", selectcolor="#1a1a2e",
                       activebackground="#1a1a2e").pack(side="right", padx=10)
    ...
    def _build_left_panel(self, parent):
        ...
        tk.Label(search_frame, text="Search:", bg="#f0f2f5",
        ...
        tk.Label(search_frame, text="Category:", bg="#f0f2f5",
        ...
        tk.Button(search_frame, text="📎 Export CSV",
                  font=("Segoe UI", 9), relief="flat",
        ...
        col_widths = {"ID": 40, "Name": 180, "Category": 100,
                      "Price (GHS)": 80, "Stock": 60, "Alert": 50,
                      "Barcode": 100, "Status": 80}
        for col in cols:
    ...
    def _build_right_panel(self, parent):
        ...
        tk.Button(right, text="✖  Clear Form", bg="#95a5a6", fg="white",
                  command=self._clear_form, **btn_cfg).pack(fill="x", pady=(12, 0))

        self.stats_frame = tk.LabelFrame(right, text="Product Stats",
                                          bg="white", fg="#333",
                                          font=("Segoe UI", 9))
        ...
        self.msg_label = tk.Label(right, textvariable=self.msg_var,
                                   font=("Segoe UI", 9),
                                   bg="white", wraplength=260)
    ...
    def _load_products(self, products=None):
        ...
        if keyword:
            products = [p for p in products if
                        keyword.lower() in p["product_name"].lower() or
                        keyword.lower() in p["category"].lower() or
                        (p["barcode"] and keyword in p["barcode"])]
        ...
        for p in products:
            ...
            status = "Active" if p.get("is_active", 1) == 1 else "Inactive"
    ...
    def _delete_product(self):
        ...
        msg   = (
            f"'{name}' has {stock} units in stock.\n\nThis product has sales history. "
            f"It will be deactivated (soft delete).\n\nProceed?"
            if stock > 0 else
            f"Remove '{name}' from the product list?\nSales history will be preserved."
        )
    ...
    def _apply_stock_adjust(self):
        ...
        ok, result = adjust_stock(self.selected_product_id, delta, reason,
    ...
    # ── Supplier Restock Dialog ────────────────────────────────────────────

    def _supplier_restock_dialog(self):
        """
        Dedicated supplier restock dialog.
        Shows all low-stock / out-of-stock products with a per-product
        restock quantity field and supplier name, then applies all at once.
        """
        ...
        header.pack(fill="x")
        tk.Label(header, text="  🚚  Supplier Restock",
                 font=("Segoe UI", 13, "bold"),
                 bg="#1a1a2e", fg="white").pack(side="left", padx=10)
        ...
        tk.Label(info_row, text="Supplier Name:",
                 font=("Segoe UI", 10, "bold"), bg="#f0f2f5").pack(side="left")
        supplier_var = tk.StringVar()
        tk.Entry(info_row, textvariable=supplier_var, font=("Segoe UI", 11),
                 relief="flat", bd=3, bg="white", width=25
                 ).pack(side="left", padx=(6, 20), ipady=5)

        tk.Label(info_row, text="Filter:",
                 font=("Segoe UI", 10), bg="#f0f2f5").pack(side="left")
        filter_var = tk.StringVar(value="Low/Out of Stock")
        filter_combo = ttk.Combobox(
            info_row, textvariable=filter_var,
        ...
        for text, w in [("Product", 220), ("Category", 100),
                         ("Current Stock", 100), ("Restock Qty", 100), ("Supplier", 120)]:
            tk.Label(col_frame, text=text, font=("Segoe UI", 9, "bold"),
                     bg="#1a1a2e", fg="#8892b0", width=w // 8,
        ...
        # Row data holders
        restock_rows = []   # list of (product, qty_var, supplier_var)
        ...
        def populate_rows(show_all=False):
            ...
            if not products:
                tk.Label(scroll_frame,
                         text="No products to display." if show_all
                         else "No low/out-of-stock products found.",
            ...
            for i, p in enumerate(products):
                ...
                # Product name
                tk.Label(row, text=p["product_name"][:28],
                         font=("Segoe UI", 10), bg=bg, anchor="w",
                         width=28).pack(side="left", padx=4)

                # Category
                tk.Label(row, text=p["category"][:12],
                         font=("Segoe UI", 9), bg=bg, fg="#666",
                         width=12, anchor="w").pack(side="left", padx=4)

                # Current stock (colored)
                stock_color = "#e74c3c" if p.get("quantity", p.get("stock", 0)) == 0 else \
                              "#e67e22" if p.get("quantity", p.get("stock", 0)) <= p.get("low_stock_alert", 5) else \
                              "#27ae60"
                current_qty = p.get("quantity", p.get("stock", 0))
                tk.Label(row, text=str(current_qty),
                         font=("Segoe UI", 10, "bold"),
                         fg=stock_color, bg=bg, width=10).pack(side="left", padx=4)

                # Restock qty entry
                qty_var = tk.StringVar(value="10")
                tk.Entry(row, textvariable=qty_var,
                         font=("Segoe UI", 10), width=8,
                         relief="flat", bd=2, bg="#fff9e6"
                         ).pack(side="left", padx=4)

                # Per-row supplier (defaults to global supplier)
                row_supplier_var = tk.StringVar()
                row_supplier_var.set(p.get("supplier", "") or "")
                tk.Entry(row, textvariable=row_supplier_var,
                         font=("Segoe UI", 9), width=14,
                         relief="flat", bd=2, bg="#f0f2f5"
                         ).pack(side="left", padx=4)

                restock_rows.append((p, qty_var, row_supplier_var))
        ...
        def apply_restock():
            ...
            for product, qty_var, row_sup_var in restock_rows:
                ...
                reason = f"Restock from {supplier_name}"
                ...
                if ok:
                    applied += 1
                    logger.info(
                        f"Restocked {product['product_name']} +{qty} "
            ...
            if applied:
                ...
                self._load_products()
                # Refresh rows to show updated stock
                populate_rows(show_all=filter_var.get() == "All Products")
        ...
        tk.Button(footer, text="✔  Apply Restock",
                  ...
                  command=apply_restock).pack(side="right", padx=(8, 0))

        tk.Button(footer, text="Close",
                  font=("Segoe UI", 10), relief="flat",
                  bg="#95a5a6", fg="white", cursor="hand2", pady=8,
    ...
    def _show_low_stock_report(self):
        ...
        text.insert(tk.END, "LOW STOCK PRODUCTS REPORT\n")
        text.insert(tk.END, "=" * 50 + "\n\n")
        ...
        try:
            ...
            for item in self.tree.get_children():
                ...
                products.append({
                    ...
                    "Category": values[2], "Price": values[3],
                    "Stock": values[4], "Alert Level": values[5],
                    "Barcode": values[6], "Status": values[7]
                })
            if not products:
