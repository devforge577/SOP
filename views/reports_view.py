...
from modules.reports import (
    get_daily_summary, get_top_products, get_payment_method_breakdown,
    get_cashier_performance, get_inventory_report, get_recent_transactions,
    get_sales_by_date_range, get_profit_analysis, get_hourly_sales_pattern,
...
class ReportsView(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        topbar.pack(fill="x")
        tk.Label(topbar, text="  📊  Reports & Analytics",
        ...
        for label, days in [("Today", 0), ("Week", 7),
                             ("Month", 30), ("Quarter", 90)]:
            tk.Button(date_presets, text=label, font=("Segoe UI", 8),
        ...
        tk.Label(date_row, text="From:", font=("Segoe UI", 9),
                 bg="#1a1a2e", fg="#8892b0").pack(side="left")
        self.start_var = tk.StringVar(
            value=(date.today() - timedelta(days=30)).strftime("%Y-%m-%d"))
        tk.Entry(date_row, textvariable=self.start_var, width=11,
        ...
        tk.Entry(date_row, textvariable=self.end_var, width=11,
                 font=("Segoe UI", 9), relief="flat"
                 ).pack(side="left", padx=(2, 8))

        tk.Button(date_row, text="Apply", font=("Segoe UI", 9),
                  bg="#e94560", fg="white", relief="flat", cursor="hand2",
                  command=self._refresh_all).pack(side="left")

        tk.Button(date_row, text="Export", font=("Segoe UI", 9),
    ...
    def _build_status_bar(self):
        ...
        self.export_label = tk.Label(status_bar, text="",
                                      font=("Segoe UI", 8),
                                      bg="#1a1a2e", fg="#27ae60")
    ...
    def _build_overview_tab(self):
        ...
        for label, key, color in [
            ...
            card = tk.Frame(self.kpi_frame, bg="white", padx=20, pady=16,
                            relief="flat", bd=1)
        ...
        tk.Label(breakdown_frame, text="Payment Method Breakdown",
                 font=("Segoe UI", 11, "bold"),
                 bg="white", fg="#1a1a2e").pack(anchor="w", pady=(0, 8))

        cols = ("Method", "Transactions", "Revenue (GHS)", "Percentage")
        self.breakdown_tree = ttk.Treeview(breakdown_frame, columns=cols,
        ...
        tk.Label(range_header, text="Daily Revenue (Date Range)",
                 font=("Segoe UI", 11, "bold"),
                 bg="white", fg="#1a1a2e").pack(side="left")
        tk.Button(range_header, text="Export Daily Data",
                  font=("Segoe UI", 8), relief="flat",
                  bg="#0f3460", fg="white", cursor="hand2",
                  command=self._export_daily_data).pack(side="right")

        range_cols = ("Date", "Transactions", "Revenue (GHS)", "Discounts", "Tax")
        self.range_tree = ttk.Treeview(range_frame, columns=range_cols,
                                        show="headings", height=8)
        for col in range_cols:
            self.range_tree.heading(col, text=col)
            self.range_tree.column(col, width=150, anchor="center")

        range_sb = ttk.Scrollbar(range_frame, orient="vertical",
        ...
        tk.Label(hourly_frame, text="Hourly Sales Pattern (Today)",
    ...
    def _build_weekly_tab(self):
        ...
        tk.Label(week_row, text="Week starting:",
                 font=("Segoe UI", 10, "bold"), bg="#f0f2f5").pack(side="left")

        # Default to start of current week (Monday)
        today      = date.today()
        monday     = today - timedelta(days=today.weekday())
        self.week_start_var = tk.StringVar(value=monday.strftime("%Y-%m-%d"))
        tk.Entry(week_row, textvariable=self.week_start_var,
                 font=("Segoe UI", 10), relief="flat", bd=3, width=12
                 ).pack(side="left", padx=(6, 8), ipady=4)

        tk.Button(week_row, text="◀ Prev Week",
                  font=("Segoe UI", 9), relief="flat",
                  bg="#0f3460", fg="white", cursor="hand2",
                  command=self._prev_week).pack(side="left", padx=2)

        tk.Button(week_row, text="This Week",
                  font=("Segoe UI", 9), relief="flat",
                  bg="#0f3460", fg="white", cursor="hand2",
                  command=self._this_week).pack(side="left", padx=2)

        tk.Button(week_row, text="Next Week ▶",
                  font=("Segoe UI", 9), relief="flat",
                  bg="#0f3460", fg="white", cursor="hand2",
                  command=self._next_week).pack(side="left", padx=2)

        tk.Button(week_row, text="Load",
                  font=("Segoe UI", 9), relief="flat",
                  bg="#e94560", fg="white", cursor="hand2",
                  command=self._load_weekly).pack(side="left", padx=(12, 0))
        ...
        for label, key, color in [
            ...
            card = tk.Frame(self.week_kpi_frame, bg="white", padx=14, pady=14,
                            relief="flat", bd=1)
        ...
        tk.Label(day_header, textvariable=self.week_title_var,
                 font=("Segoe UI", 11, "bold"),
                 bg="white", fg="#1a1a2e").pack(side="left")
        tk.Button(day_header, text="Export Week",
                  font=("Segoe UI", 8), relief="flat",
                  bg="#27ae60", fg="white", cursor="hand2",
                  command=self._export_week).pack(side="right")

        day_cols = ("Day", "Date", "Transactions",
                    "Revenue (GHS)", "Discounts (GHS)", "Tax (GHS)")
        self.week_tree = ttk.Treeview(day_frame, columns=day_cols,
                                       show="headings", height=9)
        col_w = {"Day": 100, "Date": 110, "Transactions": 110,
                 "Revenue (GHS)": 130, "Discounts (GHS)": 130, "Tax (GHS)": 100}
        for col in day_cols:
        ...
        week_sb = ttk.Scrollbar(day_frame, orient="vertical",
        ...
        tk.Label(totals_row, textvariable=self.week_totals_var,
                 font=("Segoe UI", 10, "bold"),
    ...
    def _load_weekly(self):
        ...
        for i, day_name in enumerate(DAY_NAMES):
            ...
            rows_data.append((day_name, day_date_str, transactions,
                               revenue, discounts, tax))

        # Insert rows with tags
        for (day_name, day_date_str, transactions,
             revenue, discounts, tax) in rows_data:
            ...
            self.week_tree.insert("", "end", tags=(tag,), values=(
                day_name,
                day_date_str,
                transactions,
                f"{revenue:.2f}",
    ...
    def _export_week(self):
        ...
        try:
            ...
            messagebox.showinfo("Export Complete",
                                f"Weekly report exported to {filename}")
    ...
    def _build_top_products_tab(self):
        ...
        tk.Label(tab, text="Best-selling products (by units sold)",
                 font=("Segoe UI", 10), bg="#f0f2f5", fg="#555"
                 ).pack(anchor="w", padx=16, pady=(12, 6))

        cols = ("Rank", "Product", "Category",
                "Units Sold", "Revenue (GHS)", "Contribution %")
        self.products_tree = ttk.Treeview(tab, columns=cols,
                                           show="headings", height=22)
        col_w = {"Rank": 50, "Product": 280, "Category": 140,
                 "Units Sold": 100, "Revenue (GHS)": 120, "Contribution %": 100}
        for col in cols:
        ...
        self.products_tree.pack(side="left", fill="both",
    ...
    def _build_inventory_tab(self):
        ...
        for label, key, color in [
            ...
            card = tk.Frame(self.inv_summary_frame, bg="white",
        ...
        cols = ("Product", "Category", "Price", "Stock",
                "Alert Level", "Status", "Last Updated")
        self.inv_tree = ttk.Treeview(tab, columns=cols,
                                      show="headings", height=16)
        col_w = {"Product": 200, "Category": 110, "Price": 80, "Stock": 60,
                 "Alert Level": 80, "Status": 100, "Last Updated": 140}
        for col in cols:
        ...
        self.inv_tree.pack(side="left", fill="both",
                            expand=True, padx=(16, 0), pady=(0, 16))
        sb.pack(side="left", fill="y", pady=(0, 16))
    ...
    def _build_transactions_tab(self):
        ...
        tk.Label(tab, text="50 most recent sales",
                 font=("Segoe UI", 10), bg="#f0f2f5", fg="#555"
                 ).pack(anchor="w", padx=16, pady=(12, 6))

        cols = ("Sale #", "Date & Time", "Cashier", "Customer",
                "Payment", "Items", "Discount", "Total (GHS)")
        self.tx_tree = ttk.Treeview(tab, columns=cols,
                                     show="headings", height=20)
        col_w = {"Sale #": 60, "Date & Time": 150, "Cashier": 140,
                 "Customer": 120, "Payment": 80, "Items": 60,
                 "Discount": 80, "Total (GHS)": 100}
        for col in cols:
        ...
        self.tx_tree.pack(side="left", fill="both",
                           expand=True, padx=(16, 0), pady=(0, 16))
        sb.pack(side="left", fill="y", pady=(0, 16))
    ...
    def _build_cashier_tab(self):
        ...
        tk.Label(tab, text="Sales performance per cashier (selected date range)",
                 font=("Segoe UI", 10), bg="#f0f2f5", fg="#555"
                 ).pack(anchor="w", padx=16, pady=(12, 6))

        cols = ("Rank", "Cashier", "Role", "Transactions",
                "Revenue (GHS)", "Avg Sale", "Unique Customers")
        self.cashier_tree = ttk.Treeview(tab, columns=cols,
                                          show="headings", height=20)
        col_w = {"Rank": 60, "Cashier": 200, "Role": 100, "Transactions": 120,
                 "Revenue (GHS)": 140, "Avg Sale": 120, "Unique Customers": 120}
        for col in cols:
        ...
        self.cashier_tree.pack(side="left", fill="both",
                                expand=True, padx=(16, 0), pady=(0, 16))
        sb.pack(side="left", fill="y", pady=(0, 16))
    ...
    def _build_profit_tab(self):
        ...
        for label, key, color in [
            ...
            card = tk.Frame(profit_frame, bg="white", padx=15, pady=12,
                            relief="flat", bd=1)
            card.pack(side="left", fill="both", expand=True, padx=4)
            var = tk.StringVar(value="—")
        ...
        tk.Label(detail_frame, text="Daily Profit Breakdown",
                 font=("Segoe UI", 11, "bold"),
                 bg="white", fg="#1a1a2e").pack(anchor="w", pady=(0, 8))

        profit_cols = ("Date", "Transactions", "Revenue",
                       "Est. COGS", "Profit", "Margin %")
        self.profit_tree = ttk.Treeview(detail_frame, columns=profit_cols,
                                         show="headings", height=12)
        for col in profit_cols:
            self.profit_tree.heading(col, text=col)
            self.profit_tree.column(col, width=120, anchor="center")

        sb = ttk.Scrollbar(detail_frame, orient="vertical",
                            command=self.profit_tree.yview)
    ...
    def _build_customers_tab(self):
        ...
        for label, key, color in [
            ...
            card = tk.Frame(cust_frame, bg="white", padx=15, pady=12,
                            relief="flat", bd=1)
            card.pack(side="left", fill="both", expand=True, padx=4)
            var = tk.StringVar(value="0")
        ...
        tk.Label(top_cust_frame, text="Top Customers by Spending",
                 font=("Segoe UI", 11, "bold"),
                 bg="white", fg="#1a1a2e").pack(anchor="w", pady=(0, 8))

        cust_cols = ("Rank", "Customer", "Phone", "Tier",
                     "Total Spent", "Purchases", "Loyalty Points")
        self.cust_tree = ttk.Treeview(top_cust_frame, columns=cust_cols,
                                       show="headings", height=12)
        col_w = {"Rank": 50, "Customer": 200, "Phone": 120, "Tier": 80,
                 "Total Spent": 120, "Purchases": 80, "Loyalty Points": 100}
        ...
        sb = ttk.Scrollbar(top_cust_frame, orient="vertical",
                            command=self.cust_tree.yview)
    ...
    def _load_overview(self, start, end):
        ...
        self.kpi_vars["revenue"].set(
            f"GHS {today.get('total_revenue', 0):.2f}")
        self.kpi_vars["transactions"].set(
            str(today.get("total_transactions", 0)))
        self.kpi_vars["avg"].set(
            f"GHS {today.get('avg_sale_value', 0):.2f}")
        self.kpi_vars["discounts"].set(
            f"GHS {today.get('total_discounts', 0):.2f}")
    ...
    def _load_inventory(self):
        ...
        for row in rows:
            ...
            if status == "OK":
                ...
                tag = "low"; counts["low"] += 1
            else:
                tag = "out"; counts["out"] += 1
    ...
    def _load_cashier_performance(self, start, end):
        ...
        for i, row in enumerate(get_cashier_performance(start, end), 1):
            self.cashier_tree.insert("", "end", values=(
                i,
                row["cashier"],
    ...
    def _load_profit_analysis(self, start, end):
        ...
        if profit_data:
            ...
            self.profit_vars["tax"].set(
                f"GHS {profit_data.get('total_tax', 0):,.2f}")
        ...
        for row in get_sales_by_date_range(start, end):
            ...
            self.profit_tree.insert("", "end", values=(
                row["sale_date"],
                row["total_transactions"],
                f"GHS {revenue:.2f}",
                f"GHS {cogs:.2f}",
    ...
    def _view_transaction_details(self, event):
        ...
        text = tk.Text(win, font=("Courier New", 10), bg="white",
                       padx=10, pady=10)
        ...
        tk.Button(win, text="Close", command=win.destroy,
                  bg="#e94560", fg="white", relief="flat", pady=5).pack(pady=10)
    ...
    def _export_current_report(self):
        ...
        tree_map = {
            "Overview":     (self.range_tree,
                             ["Date","Transactions","Revenue (GHS)","Discounts","Tax"]),
            "Weekly":       (self.week_tree,
                             ["Day","Date","Transactions","Revenue (GHS)",
                              "Discounts (GHS)","Tax (GHS)"]),
            "Top Products": (self.products_tree,
                             ["Rank","Product","Category","Units Sold",
                              "Revenue (GHS)","Contribution %"]),
            "Inventory":    (self.inv_tree,
                             ["Product","Category","Price","Stock",
                              "Alert Level","Status","Last Updated"]),
            "Transactions": (self.tx_tree,
                             ["Sale #","Date & Time","Cashier","Customer",
                              "Payment","Items","Discount","Total (GHS)"]),
        }

        matched = next(
            ((tree, headers)
             for key, (tree, headers) in tree_map.items() if key in tab_name),
            None
        )
        if not matched:
    ...
    def _export_daily_data(self):
        data = [self.range_tree.item(item)["values"]
                for item in self.range_tree.get_children()]
        if not data:
            messagebox.showinfo("Export", "No data to export."); return

        filename = (f"daily_sales_{self.start_var.get()}"
                    f"_to_{self.end_var.get()}.csv")
        ...
        try:
            ...
            self.after(3000, lambda: self.export_label.config(text=""))
            messagebox.showinfo("Export Complete",
                                f"Daily data exported to {filename}")
        except Exception as e:
            messagebox.showerror("Export Error", f"Failed to export: {str(e)}")